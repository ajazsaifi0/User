...
class _SignUpState extends State<SignUp> {
  ...
  _getProfileFromGallery() async {
    PickedFile? DP = await ImagePicker().getImage(
    ...
    if (DP != null) {
      setState(() {
       Profile=File(DP.path);
        BaseProfile = base64Encode(Profile!.readAsBytesSync());
      });
  ...
  UploadData() async{
    var requestBody={
      ...
      "dob":formattedDate,
      "user_image":BaseProfile
    };
    dynamic response= await api.ApiCallingWithRequestBody("/user/upload.php", requestBody);
                    ...
                    GestureDetector(
                      child: Container(
                        ...
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Profile==null?Icon(Icons.add):
                            Container(),
                            Profile==null? Text("Upload Front Photo PAN",style: TextStyle(fontSize: 16,fontWeight: FontWeight.w300),textAlign: TextAlign.center,):
                            Image.file(File(Profile!.path),height: 120,width: 120,)
                          ],
                        ),
                      ),
                      onTap: (){
